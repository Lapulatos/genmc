...
case "$output_root" in
	/data3/sujie/experiments/caat-optimization/p0-regional-rvf-opportunity-51-*) ;;
...
mkdir -p "$output_root"
docker run --rm --privileged --cpuset-cpus=0-7 --memory=100g --memory-swap=100g \
...
[[ "$(bzcat "${result_xmls[0]}" | grep -c '<run ')" -eq 51 ]] || {
	echo "opportunity census did not execute exactly 51 tasks" >&2
	exit 3
}
