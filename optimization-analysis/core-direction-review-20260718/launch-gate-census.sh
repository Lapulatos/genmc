...
definition="$source_root/optimization-analysis/core-direction-review-20260718/gate-census.xml"
out="${1:?pass a new gate-census result directory}"

[[ "$out" == "$root/formal-results/gate-census-"* && ! -e "$out" ]] || exit 2
...
rewrite="/workspace/experiments/caat-optimization/gate-census-rewrite/$(basename "$out")/control"
mkdir -p "$rewrite"
status=0
env GENMC_BINARY="$wrapper" GENMC_REAL_BINARY="$binary" \
  GENMC_EXPERIMENT_MODE=control GENMC_REWRITE_ROOT="$rewrite" \
  benchexec --no-container --numOfThreads 48 --allowedCores 0-23,28-51 \
  --outputpath "$out/control/" "$definition" >"$out/control/console.log" 2>&1 || status=$?
