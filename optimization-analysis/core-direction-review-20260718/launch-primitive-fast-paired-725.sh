...
[[ "$out" == "$root/formal-results/primitive-fast-compose-cycle-paired-725-"* &&
   ! -e "$out" ]] || exit 2
[[ -x "$binary" && -x "$wrapper" && -f "$definition" ]] || exit 2
mkdir -p "$out/baseline/run" "$out/candidate/run"
sha256sum "$binary" "$wrapper" "$definition" \
  /data3/sujie/experiments/caat-optimization/rvf-opportunity-census/definitions/fair-census-all.set \
...
run_one()
{
	local mode="$1" cores="$2" target="$3"
	local rewrite="/workspace/experiments/caat-optimization/primitive-fast-725-rewrite/$(basename "$out")/$mode"
	mkdir -p "$rewrite"
	GENMC_EXPERIMENT_MODE="$mode" GENMC_REWRITE_ROOT="$rewrite" \
	benchexec --no-container --numOfThreads 24 --allowedCores "$cores" \
		--rundefinition baseline --outputpath "$target/run/" "$definition" \
		>"$target/run/console.log" 2>&1
}

baseline_status=0
candidate_status=0
run_one baseline 0-23 "$out/baseline" & baseline_pid=$!
run_one primitive-cache-fast-compose-cycle 28-51 "$out/candidate" & candidate_pid=$!
