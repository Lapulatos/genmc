...
definition="$source_root/optimization-analysis/core-direction-review-20260718/regional-rvf-paired-725.xml"
...
case "$output_root" in
	/data3/sujie/experiments/caat-optimization/p0-regional-rvf-paired-725-*) ;;
...
[[ "$threads_per_lane" =~ ^[1-9][0-9]*$ ]] || {
	echo "GENMC_THREADS_PER_LANE must be a positive integer" >&2
	exit 2
}
(( $(nproc) >= 2 * threads_per_lane )) || {
	echo "host needs at least $((2 * threads_per_lane)) logical CPUs" >&2
	exit 2
}
...
[[ -f "$set_file" && "$(grep -cve '^[[:space:]]*$' "$set_file")" -eq "$expected" ]] || {
	echo "full task set must exist and contain exactly $expected rows: $set_file" >&2
	exit 2
}
...
run_lane baseline baseline "$baseline_cores" & baseline_pid=$!
run_lane candidate regional-rvf "$candidate_cores" & candidate_pid=$!
baseline_status=0
candidate_status=0
...
for lane in baseline candidate; do
	mapfile -t archives < <(find "$output_root/$lane" -name '*.logfiles.zip')
	mapfile -t result_xmls < <(find "$output_root/$lane" -name '*.xml.bz2')
	[[ ${#archives[@]} -eq 1 && ${#result_xmls[@]} -eq 1 ]] || {
		echo "$lane must contain exactly one log archive and result XML" >&2
		exit 3
	}
	[[ "$(bzcat "${result_xmls[0]}" | grep -c '<run ')" -eq "$expected" ]] || {
		echo "$lane did not execute exactly $expected tasks" >&2
		exit 3
	}
done
