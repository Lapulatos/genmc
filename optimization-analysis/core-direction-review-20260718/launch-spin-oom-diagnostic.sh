...
definition="$source_root/optimization-analysis/core-direction-review-20260718/spin-oom-diagnostic.xml"
out="${1:?pass a new spin diagnostic result directory}"

[[ "$out" == "$root/formal-results/spin-oom-diagnostic-"* && ! -e "$out" ]] || exit 2
[[ -x "$binary" && -x "$wrapper" && -f "$definition" ]] || exit 2
! pgrep -af '/bin/[g]enmc|[b]enchexec' || exit 2
mkdir -p "$out/baseline"
sha256sum "$binary" "$wrapper" "$definition" "$pipeline/tools/rewrite_sources.py" >"$out/input-sha256.txt"
...
rewrite="/workspace/experiments/caat-optimization/spin-oom-diagnostic-rewrite/$(basename "$out")"
mkdir -p "$rewrite"
status=0
env GENMC_BINARY="$wrapper" GENMC_REAL_BINARY="$binary" GENMC_EXPERIMENT_MODE=baseline \
  GENMC_REWRITE_ROOT="$rewrite" benchexec --no-container --numOfThreads 4 \
  --allowedCores 0-3 --outputpath "$out/baseline/" "$definition" \
  >"$out/baseline/console.log" 2>&1 || status=$?
