...
definition="$source_root/optimization-analysis/core-direction-review-20260718/finite-symbolic-pthread-wmm.xml"
out="${1:?pass a new finite-symbolic-pthread-wmm result directory}"

[[ "$out" == "$root/formal-results/finite-symbolic-pthread-wmm-"* && ! -e "$out" ]] || exit 2
...
run_one() {
  local configuration=$1 cores=$2 result="$out/$1"
  local rewrite="/workspace/experiments/caat-optimization/finite-symbolic-pthread-rewrite/$(basename "$out")/$configuration"
