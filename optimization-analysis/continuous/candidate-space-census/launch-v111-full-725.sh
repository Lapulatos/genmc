...
[[ "$out" == "$root/formal-results/v111-full-725-"* && ! -e "$out" ]] || exit 2
...
run_id=$(basename "$out")
v9_rewrite="/workspace/experiments/caat-optimization/v111-full-rewrite/$run_id/v9"
v11_rewrite="/workspace/experiments/caat-optimization/v111-full-rewrite/$run_id/v11"
[[ ! -e "$v9_rewrite" && ! -e "$v11_rewrite" ]] || exit 2
