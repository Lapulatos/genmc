...
For every finite relation `R`, both `acyclic(R+)` and `irreflexive(R+)` are equivalent
to `acyclic(R)`. When a named `TransitiveClosure` predicate has no predicate consumers
and all of its check consumers are one of these two cycle tests, the CAT consistency
query can canonicalize them to `acyclic(R)` and remove the now-unobservable closure
predicate. This avoids materializing the full closure without changing candidate
enumeration or CAT consistency.
...
- At least one check consumes the predicate, and every such check is `acyclic` or
  `irreflexive`; both normalize to `acyclic` on the seed while retaining check name and
  source span.
- No normalized predicate consumes the closure value.
- `empty`, downstream predicate use, reflexive closure, and ambiguous
...
1. Independent random-relation property: `acyclic(R)`, `acyclic(R+)`, and
   `irreflexive(R+)` agree for finite relations, including self loops, multi-edge
   cycles, DAGs, and disconnected graphs.
2. Exact activation tests for one and multiple acyclic checks.
3. Near-neighbor tests proving no activation under downstream use, `empty`, or
   reflexive closure.
