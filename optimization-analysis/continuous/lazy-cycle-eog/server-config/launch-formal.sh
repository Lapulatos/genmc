...
root=/data3/sujie/experiments/caat-optimization/lazy-cycle-eog
...
for repetition in 1 2 3 4; do
	...
	pids=(); variants=(); model_indices=()
	started=$(date --iso-8601=seconds)
	for index in 0 1 2; do
		before_cores="${before_ranges[$index]}"
		after_cores="${after_ranges[$index]}"
		if ((repetition % 2 == 0)); then
			temporary="$before_cores"; before_cores="$after_cores"; after_cores="$temporary"
		fi
		for variant in before after; do
			binary="$root/build-before/bin/genmc"; cores="$before_cores"
			if [[ "$variant" == after ]]; then
				binary="$root/build-after/bin/genmc"; cores="$after_cores"
			fi
			model="${models[$index]}"
			...
			pids+=("$!"); variants+=("$variant"); model_indices+=("$index")
		done
	done
	...
	failed=0
	for job in "${!pids[@]}"; do
		exit_code=0; wait "${pids[$job]}" || exit_code=$?
		index="${model_indices[$job]}"; variant="${variants[$job]}"
		statuses["$index-$variant"]="$exit_code"
		((exit_code == 0)) || failed=1
	done
	finished=$(date --iso-8601=seconds)
	for index in 0 1 2; do
		before_cores="${before_ranges[$index]}"; after_cores="${after_ranges[$index]}"
		if ((repetition % 2 == 0)); then
			temporary="$before_cores"; before_cores="$after_cores"; after_cores="$temporary"
		fi
		printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$repetition" \
			"${models[$index]}" "$before_cores" "$after_cores" \
