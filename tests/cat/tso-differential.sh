...
run_case()
{
	...
	if [[ "${mode}" == cat ]]; then
		...
		case_output="$("${genmc}" --tso --disable-estimation --disable-mm-detector \
			"${program}" 2>&1)"
	fi
	case_status=$?
	set -e
...
# Compare status, complete executions, verdict, and a representative diagnostic class.
compare_case()
{
	relative_program="$1"
	expected_count="$2"
	...
	if [[ "${builtin_status}" != "${cat_status}" || "${builtin_count}" != "${cat_count}" ||
		...
		echo "TSO mismatch for ${relative_program}: status ${builtin_status}/${cat_status}," \
...
# SB distinguishes TSO from SC; the remaining cases cover dependencies,
# message passing, same-location order, RMW atomicity, warnings, and errors.
compare_case "correct/litmus/SB/variants/sb0.c" 4 "No errors were detected."
