...
- Phase 1.5 maps one immutable `ExecutionGraph` snapshot to dense IDs in insertion order, excluding only internal `EmptyLabel` placeholders. Unclassified concrete labels remain in `_` and participate in `po/int/ext`; emitted NA reads/writes remain in `R/W`.
